...
const clickoutside = {
  ...
  mounted(e, t) {
    ...
    e.addEventListener("mousedown", (n) => {
      const { width: c, height: d } = e.getBoundingClientRect(), i = e.offsetLeft, s = e.offsetTop, y = n.clientX, b = n.clientY, v = i, p = r - i - c, f = s, g = l - s - d;
      document.onmousemove = (_) => {
        const a = _.clientX - y, u = _.clientY - b;
        return a < 0 && a <= -v ? e.style.left = i - v + "px" : a > 0 && a >= p ? e.style.left = i + p + "px" : e.style.left = i + a + "px", u < 0 && u <= -f ? e.style.top = s - f + "px" : u > 0 && u >= g ? e.style.top = s + g + "px" : e.style.top = s + u + "px", t == null || t.value(_), !1;
      }, document.onmouseup = () => {
